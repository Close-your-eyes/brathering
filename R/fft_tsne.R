...
#'
#' @param X numeric matrix with observations as rows
#' @param dims dimensionality of the embedding
#' @param perplexity perplexity is used to determine the bandwidth of the
#' Gaussian kernel in the input space
#' @param theta Set to 0 for exact. If non-zero, then will use either
#' Barnes Hut or FIt-SNE based on fft_not_bh. If Barnes Hut, then
#' this determines the accuracy of BH approximation.
#' @param max_iter Number of iterations of t-SNE to run
#' @param fft_not_bh if theta is nonzero, this determines whether to
...
#' @param stop_early_exag_iter When to switch off early exaggeration.
#' @param exaggeration_factor coefficient for early exaggeration (>1)
#' @param no_momentum_during_exag Set to 0 to use momentum
#' and other optimization tricks. 1 to do plain, vanilla
#' gradient descent (useful for testing large exaggeration
#' coefficients)
#' @param start_late_exag_iter When to start late exaggeration. 'auto' means
...
#' @param late_exag_coeff Late exaggeration coefficient.
#' Set to -1 to not use late exaggeration.
#' @param mom_switch_iter Iteration after which the final momentum is used
#' @param momentum Momentum used in the first part of the optimization
#' @param final_momentum Momentum used in the final part of the optimization
#' @param learning_rate Set to desired learning rate or 'auto', which
...
#'      off.
#' @param load_affinities If 1, input similarities are loaded from a file and not computed
#'  If 2, input similarities are saved into a file.
#'  If 0, affinities are neither saved nor loaded
#' @param fast_tsne_path path to fast_tsne binary
#' @param nthreads Number of threads to use. Setting to 0 corresponds to detecting and using all available cores.
#' May require OpenMP.
...
#'      details.
#'
#' @returns matrix with tsne dimensions
#' @export
#'
#' @examples
fft_tsne <- function(
  ...
  colnames(Yout) <- paste0("tSNE_", 1:ncol(Yout))
  rownames(Yout) <- X_rownames
  return(Yout)
}
